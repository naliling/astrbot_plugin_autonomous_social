...
def _at_days_ahead(now: float, days: int, hour: int, minute: int) -> float:
...
def _next_weekday(now: float, weekday: int, hour: int = 10, minute: int = 0) -> float:
...
def _anchor_due(now: float, pattern: str, days: int, hour: int, minute: int) -> float:
    """把锚点算成一个具体时间点（本地时区）。"""
    if days == -1:
        return _next_weekday(now, 5, 12, 0)
    if days == 0 and hour == 0 and minute == 0:
        return now + _CUE_REL_SECONDS
    return _at_days_ahead(now, days, hour, minute)


def extract_cue(text: str, now: Optional[float] = None) -> Tuple[Optional[str], float]:
    """从对方消息里提取可跟进的时间锚点。
    ...
    for pattern, days, hour, minute in _CUE_ANCHORS:
        ...
        if not match:
            continue
        candidates.append((match.start(), pattern, _anchor_due(now, pattern, days, hour, minute)))
    ...
    if wd:
        ...
        candidates.append((wd.start(), "weekday", _next_weekday(now, weekday)))
...
def generate_reason(
    ...
    slot = time_slot(datetime.fromtimestamp(now).hour)
