...
_NIGHT_SLOTS = {"late_night", "deep_night", "early_morning"}
...
class MessageGenerator:
    ...
    async def _compose(
        ...
        *,
        ...
        # 跟进断掉的话与「另起一个话题」要的东西相反：那条就是要问一句
        is_followup = str((reason_meta or {}).get("intent") or "") == "followup"
        max_len = (
        ...
        def assemble(dropped: frozenset) -> str:
            ...
            if is_followup:
                parts.append("就是想知道人还在不在，或者是不是被什么事叫走了。一句话就够了。")
