...
class SocialEngine:
    ...
    def followup_gate(
        ...
        """跟进断话的闸门。与 gate_reason 的区别只有一处：不管 recent_talk。

        真人追问恰恰发生在「刚聊完」之后，拿「不久前才说过话」去挡它，等于把
        这条路径永远堵死。同理不查对方作息：TA 十几分钟前还在说话，这个点肯走醒着。
        """
        if body and body.get("asleep"):
            return "她正在睡觉"
        last_seen = float(u.get("last_seen", 0) or 0)
        if last_seen > 0 and float(u.get("followup_for", 0) or 0) == last_seen:
            return "这段沉默已经问过一回，再问就是催"
        if now - float(u.get("followup_at", 0) or 0) < self.cfg.followup_cooldown_minutes * 60:
            return "刚跟进过一次，给 TA 点时间"
        if self.cfg.in_quiet_hours(self._moment_of(bid, now).hour):
            return "现在是安静时段"
        return ""

    def _dangling_windows(self) -> Tuple[float, float, float]:
        """跟进的三个时间窗：多久算断了 / 最长追到多久 / 之前多久内得有过话。"""
        after = self.cfg.followup_after_minutes * 60
        return after, self.cfg.followup_max_minutes * 60, max(600.0, after * 3)
    ...
    def _dangling_pick(
        ...
        if not self.cfg.followup_enabled:
            return None
        after, ceiling, context = self._dangling_windows()
        ...
        for uid, u in self.state.bot(bid).get("users", {}).items():
            ...
            if not reason:
                continue
            if self.followup_gate(bid, uid, u, now, body):
                continue
            gap = now - float(u.get("last_seen", 0) or 0)
            if best is None or gap < best[0]:
                best = (gap, uid, u, reason)
        if best is None:
            return None
    ...
    async def try_once(self) -> None:
        ...
        danglings: List[Tuple[str, str, Dict[str, Any], str, float]] = []
        max_streak = 0
        top_desire = None
        ...
        for bid in bots:
            ...
            dangling = self._dangling_pick(bid, now, bot_state)
            if dangling:
                danglings.append((bid, *dangling))
        ...
        if not contenders and not danglings:
            self.log("没有人攒够念头，本轮只是把时间补算上")
            return

        # 全局护栏对两条路一视同仁：同一轮里不能既追问又另起话题
        ...
        for bid, uid, u, reason, gap in sorted(danglings, key=lambda item: item[4]):
            left = guard_left(bid)
            if left > 0:
                self.log(f"{bid} 全局护栏未过（还需 {left / 60:.0f} 分钟），跟进也先不发")
                continue
            sent, note = await self._speak(
                bid, uid, u, 0.0, now, preset=(reason, dangling_meta())
            )
            self.log(f"跟进 {uid}（沉默 {gap / 60:.0f} 分钟）urge=0 → {note}")
            if sent:
                return
    ...
    async def _speak(
        ...
        *,
        ...
        if reason_meta and reason_meta.get("intent") == "followup":
            # 记下这段沉默已经问过一回：对方再不回，也不能追第二遍
            u["followup_for"] = float(u.get("last_seen", 0) or 0)
            u["followup_at"] = sent_ts
    ...
    def urge_panel_text(self, limit: int = 6) -> str:
        ...
        for bid, uid, urge, interest, pend, seen in rows[:limit]:
            ...
            lines.append(
                ...
                f"{self._dangling_mark(bid, uid, seen, now)}"
            )
        if len(rows) > limit:
            lines.append(f"  …另有 {len(rows) - limit} 人未列出")
        return "\n".join(lines)

    def _dangling_mark(self, bid: str, uid: str, seen: float, now: float) -> str:
        """状态面板上标出「话正断着」：能跟、已跟过、还是不该跟。"""
        if not self.cfg.followup_enabled or seen <= 0:
            return ""
        u = self.state.user(bid, uid)
        silence = (now - seen) / 60.0        # 分钟
        after, ceiling, context = self._dangling_windows()   # 秒
        if silence * 60 < after:
            return ""
        reason = dangling_reason(
            u,
        ...
        if not reason:
            return ""
        if float(u.get("followup_for", 0) or 0) == seen:
            return f"｜话断了 {silence:.0f} 分钟（已问过一回）"
        why = self.followup_gate(bid, uid, u, now)
        return f"｜话断了 {silence:.0f} 分钟（{'待跟进' if not why else '不跟：' + why}）"
    ...
    async def status_text(self) -> str:
        ...
        return (
            ...
            f"断话跟进：{'开（对方沉默 ' + str(self.cfg.followup_after_minutes) + ' 分钟问一句，一段只问一次）' if self.cfg.followup_enabled else '关'}\n"
